...
class UserParcellsToDisplay extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    async readFile(uri, filename) {
        try {
            ...
            if (fileInfo.size > 500000) {
                ...
                let fileBuff = Buffer.from(fileString);
                this.uploadFile(filename, fileBuff);
            }
        } catch (err) {
            console.warn(err);
        }
...
export default UserParcellsToDisplay;
